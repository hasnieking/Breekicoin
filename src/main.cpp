...
#if defined(NDEBUG)
# error "Breekicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0e56ff7a1f3da9324a12b111f257b358e6a75166aa774972aece91f8e9b55b6d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Breekicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Breekicoin Signed Message:\n";
...
{
    // Breekicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Breekicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5.0 * 60; // Breekicoin: 5 minutes
static const int64 nTargetSpacing = 0.5 * 60; // Breekicoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Breekicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Breekicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0e56ff7a1f3da9324a12b111f257b358e6a75166aa774972aece91f8e9b55b6d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Breekicoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1514928813;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1206462;
        ...
        assert(block.hashMerkleRoot == uint256("0xeaadd5cf587ac564843aa3cf2124e0f199f86ecd13fcd7652143bfb002b36a2a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Breekicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BreekicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BreekicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BreekicoinMiner(CWallet *pwallet)
{
    printf("BreekicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("breekicoin-miner");
    ...
    try { loop {
        ...
        printf("Running BreekicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BreekicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BreekicoinMiner, pwallet));
}
